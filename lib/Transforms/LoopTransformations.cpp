...
LogicalResult runDataLayout(func::FuncOp &f, DataLayoutOp &dataLayoutOp,
                           Value &array) {
  // 1) Get the schedule
  // auto memref = partitionOp.target(); // return a Value type
  auto kind = dataLayoutOp.partition_kind();
  unsigned int target_dim = dataLayoutOp.dim();
  auto optional_factor = dataLayoutOp.factor();
  int factor = 0;
  if (optional_factor.hasValue()) {
    factor = optional_factor.getValue();
  } else {
    factor = -1;
    if (kind != PartitionKindEnum::CompletePartition) {
      dataLayoutOp.emitError("Should pass in `factor' for array partition");
      return failure();
    }
  }
  ...
  unsigned rank = arrayType.getRank();
  if (layout.getNumResults() != rank) {
    dataLayoutOp.emitWarning("Partition on the array partitioned before. "
                            "The original layout map will be rewritten!");
  }
  for (int64_t dim = 0; dim < rank; ++dim) {
    if (target_dim == 0 || (target_dim > 0 && dim == target_dim - 1)) {
      if (kind == PartitionKindEnum::CyclicPartition) {
        ...
        dataLayoutOp.emitError("Invalid reshaping kind");
        return failure();
      }
    } else {
      if (layout.getNumResults() == rank) {
...
bool isHCLOp(Operation &op) {
  return llvm::isa<SplitOp, TileOp, ReorderOp, UnrollOp, PipelineOp, ParallelOp,
                   FuseOp, ComputeAtOp, PartitionOp, DataLayoutOp, ReuseAtOp, BufferAtOp,
...
bool applyLoopTransformationOnSingleFunction(
  ...
  for (Operation &op : f.getOps()) {
    if (isHCLOp(op)) {
      ...
      } else if (auto new_op = dyn_cast<DataLayoutOp>(op)) {
		Value array;
		if (findArray(f, new_op.target(), array)) {
	  	  if (failed(runDataLayout(f, new_op, array)))
	    	return false;
	} else {
	return false;
	}
