...
private:
  ...
  void emitLoopDirectives(Operation *op);
	void emitDataLayoutDirectives(Operation *op);
...
void ModuleEmitter::emitDataLayoutDirectives(Operation *op) {
  std::string target = op->getAttr("target").cast<StringAttr>().getValue().str();
	std::string type = op->getAttr("layout_type").cast<StringAttr>().getValue().str();
	int dim = op->getAttr("dim").cast<IntegerAttr>().getInt();
	int factor = op->getAttr("factor").cast<IntegerAttr>().getInt();

	indent();
	os << "#pragma HLS array_reshape";
	os << " variable=" << target;
